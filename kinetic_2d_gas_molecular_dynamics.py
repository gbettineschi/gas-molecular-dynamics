...
vmin = 0; vmax = 1; # used for defining the size of the display window
...
def initialize(ms, rs=None, vs=None, v=0., initialization="only_one"):
...
class Wall:
    ...
    def solve_collision(self, rs, vs, ms, i_next):
        ...
        if M < np.inf:
            ...
            vs[i_next,ind] = 2*M/(m+M) * self.v + (m-M)/(M+m) * vs[i_next,ind]
            self.v += dv_wall
        else:
    ...
    def plot(self):
        ...
        if orientation == "x":
            plt.plot([0.,1.,1.,0.], [r-w/2, r-w/2, r+w/2, r+w/2], c="k")
        else:
            plt.plot([r-w/2, r-w/2, r+w/2, r+w/2], [0.,1.,1.,0.], c="k")
...
def update_next_collision_matrix(rs, vs, walls, a, b, c, delta, tm, tnext, i_next, j_next, t_next_coll):
    ...
    N = rs.shape[0]; n_walls = len(walls)
    ...
    elif walls[j_next-N].M < np.infty:
...
def trajectory(ms, seed=None, rs=None, vs=None, 
               v=5., initialization="only_one",
               T=100000, plot_time=10):
    N = len(ms)
    ...
    walls = [Wall("x", 0., np.inf, 0.), Wall("x", 1., np.inf, 0.), Wall("y", 0., np.inf, 0.), Wall("y", 1., np.inf, 0.)]
    
    # UNCOMMENT TO ADD A MOVING WALL in the box with params (orientation, position, mass, width)
    # walls.append(Wall("y", 0.5, 100, 0.01))
    
    a, b, c, delta, tm, tnext, i_next, j_next, t_next_coll = next_collision_matrix(rs, vs, walls)
    ...
    for t in range(T):
        ...
        elapsed = 0. # time passed since the last "epoch", i.e. a time step 'dt'
        while (elapsed + t_next_coll) < dt:
            ...
            i_next, j_next, t_next_coll = update_next_collision_matrix(rs, vs, walls, a, b, c, delta, tm, tnext, i_next, j_next, t_next_coll)
        ...
        if t % plot_time == 0:
            ...
            s = ((f1.get_window_extent().width  / (vmax-vmin+0.1) * 110./f1.dpi) ** 2) * R**2
    ...
    plt.show(block=True) # when the loop is finished, we show the plot one last time in a blocking way to keep the window open
    return rs, vs                

plt.close("all")

f1 = plt.figure(figsize=(6., 8), dpi=120)
...
rs, vs = trajectory(ms, T=2000, plot_time=2, v=5., seed=3234, initialization="uniform")
# backwards in time
